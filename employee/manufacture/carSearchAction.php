...
extract($_POST);
$sql = "SELECT * FROM car_details WHERE chassis_number = '$chassis_number'";
$result=mysqli_query($conn,$sql);
$rows=mysqli_fetch_all($result,MYSQLI_ASSOC);
foreach ($rows as $row) {
    echo "<div><table><tr><td>Chassis Number: </td><td>" . $row['chassis_number']
    . "</td></tr><tr><td>Color: </td><td>" . $row['color'] 
    . "</td></tr></div>";
}
$sql = "SELECT * FROM model_specifications WHERE m_id in (select m_id from car_details WHERE chassis_number = '$chassis_number')";
$result=mysqli_query($conn,$sql);
$rows=mysqli_fetch_all($result,MYSQLI_ASSOC);
foreach ($rows as $row) {
