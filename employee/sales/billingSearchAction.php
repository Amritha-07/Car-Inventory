...
foreach ($rows as $row) {
    echo "<div><table><tr><td>Billing ID: </td><td>" . $row['b_id']
    . "</td></tr><tr><td>Mode: </td><td>" . $row['b_mode'] 
    . "</td></tr><tr><td>Status: </td><td>" . $row['b_status'] 
    . "</td></tr></div>";
}
$sql = "SELECT * FROM customer_details WHERE c_id in (select c_id from billing_details WHERE b_id = '$b_id')";
$result=mysqli_query($conn,$sql);
$rows=mysqli_fetch_all($result,MYSQLI_ASSOC);
foreach ($rows as $row) {
...
$sql = "SELECT * FROM employee_details WHERE e_id in (SELECT e_id from billing_details WHERE b_id = '$b_id')";
$result=mysqli_query($conn,$sql);
